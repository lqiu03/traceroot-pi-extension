...
export function registerLlm(rt: Runtime): void {
  ...
  safeOn(rt, 'message_end', async (raw) => {
    ...
    // Response text is conversation content: when captureContent is off it is neither
    // exported nor retained in state (lastAssistantText feeds the session Output).
