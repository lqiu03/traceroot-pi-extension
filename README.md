...
| `TRACEROOT_CAPTURE_CONTENT`                              | `true`                                     | Capture prompt and response text on the session/turn/LLM Input/Output panels. Set `false` to keep conversation content on-machine entirely: spans then carry only metadata (timing, tokens, counts, error state).                                                                                                                                  |
...
the trace view. Prompt/response text is controlled by
`TRACEROOT_CAPTURE_CONTENT`, tool arguments/results (and the descriptive tool
span-name suffix) by `TRACEROOT_CAPTURE_TOOL_IO`, and raw payloads by
`TRACEROOT_CAPTURE_FULL_PAYLOAD` — with all three off, spans carry only
metadata.
