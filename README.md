...
| `TRACEROOT_CAPTURE_FULL_PAYLOAD`                         | `false`                                    | Capture full LLM request payloads (`traceroot.pi.full_request_payload`, 16 KB cap), the request-message Input panel (`traceroot.span.input`), raw typed input (`traceroot.pi.raw_input`), and provider error text in span statuses. Off by default: LLM spans then record only `request_message_count`, not conversation content. May contain PII.  |
| `TRACEROOT_CAPTURE_CONTENT`                              | `true`                                     | Capture prompt and response text on the session/turn Input/Output panels and the LLM Output panel. Set `false` to keep this conversation content on-machine: spans then carry only metadata (timing, tokens, counts, error state). (The LLM _Input_ panel — the request messages — is governed by `TRACEROOT_CAPTURE_FULL_PAYLOAD`, not this flag.) |
| `TRACEROOT_CAPTURE_TOOL_IO`                              | `true`                                     | Capture tool-call arguments and results (truncated) on tool spans, and the command/path suffix in tool span names (`bash: npm test`). Set `false` to record only the bare tool name, error state, and duration.                                                                                                                                     |
...
the trace view. Session/turn Input and Output, and the LLM Output panel, are
controlled by `TRACEROOT_CAPTURE_CONTENT`; the LLM Input panel (the request
messages) and raw payloads by `TRACEROOT_CAPTURE_FULL_PAYLOAD`; tool
arguments/results and the descriptive tool span-name suffix by
`TRACEROOT_CAPTURE_TOOL_IO` — with all three off, spans carry only metadata.
...
is time-bounded (with the exporter's own request deadline inside that bound), so
a hung exporter can never stall pi's exit; non-quit session transitions use a
shorter flush deadline so an offline network never makes `/new` or reload feel
stuck. If the process exits without a session-shutdown event, a `beforeExit`
fallback closes open spans and flushes best-effort. Export failures and dropped
spans are reported through the debug log/stderr when `TRACEROOT_PI_DEBUG` or
`TRACEROOT_LOG_FILE` is set. Open spans are closed in reverse nesting order on
the way out.

## Advanced tuning

The exporter and batch processor are the stock OpenTelemetry ones, so the
standard `OTEL_*` environment variables apply and override the built-in
defaults:

- **Sampling** — `OTEL_TRACES_SAMPLER` / `OTEL_TRACES_SAMPLER_ARG` (e.g.
  `traceidratio` with arg `0.2` to keep 20% of traces). Useful for long
  autonomous sessions that would otherwise export every span.
- **Batching** — `OTEL_BSP_MAX_QUEUE_SIZE`, `OTEL_BSP_MAX_EXPORT_BATCH_SIZE`,
  `OTEL_BSP_EXPORT_TIMEOUT`. The built-in batch size is already reduced when
  `TRACEROOT_CAPTURE_FULL_PAYLOAD` is on, so large payloads stay under common
  ingest body limits.
- **Export timeout** — `OTEL_EXPORTER_OTLP_TRACES_TIMEOUT` /
  `OTEL_EXPORTER_OTLP_TIMEOUT`. The built-in default keeps the request deadline
  inside the shutdown budget so a hung endpoint cannot stall pi's exit.
